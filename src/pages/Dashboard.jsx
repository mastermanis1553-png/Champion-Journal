...
export default function Dashboard() {
  ...
  const MetricCard = ({ title, value, sub, color="text-[#1a1a2e]" }) => (
    <div className="bg-white/70 backdrop-blur-md border-2 border-[#d0bdf4] p-7 rounded-2xl shadow-lg hover:shadow-xl transition-all hover:border-[#8458B3]">
      <p className="text-xs font-black text-[#8458B3] uppercase tracking-wider mb-3" style={{ fontFamily: 'Poppins, sans-serif' }}>{title}</p>
      <h2 className={`text-3xl font-black ${color}`} style={{ fontFamily: 'Poppins, sans-serif', letterSpacing: '-0.7px' }}>{value}</h2>
      <p className="text-xs font-semibold text-[#a28089] mt-2 uppercase tracking-wider" style={{ fontFamily: 'Poppins, sans-serif' }}>{sub}</p>
    </div>
  );
  ...
  return (
    <div className="space-y-8">
      <div className="mb-10">
        <h1 className="text-4xl font-black text-[#8458B3] tracking-tight uppercase" style={{ fontFamily: 'Poppins, sans-serif', letterSpacing: '-1px' }}>
          Trading <span className="text-[#a0d2eb]">Terminal</span>
        </h1>
        <p className="text-[#a28089] text-sm mt-3 font-medium" style={{ fontFamily: 'Inter, sans-serif' }}>Institutional-grade performance overview and analytics</p>
      </div>
      ...
      <div className="bg-white/70 backdrop-blur-md border-2 border-[#d0bdf4] p-8 rounded-2xl text-center shadow-lg">
        <p className="text-xs font-black text-[#8458B3] uppercase tracking-wider mb-3" style={{ fontFamily: 'Poppins, sans-serif' }}>Total Open Risk (TOR) Exposure</p>
        <h2 className={`text-4xl font-black ${m.tor > 3 ? 'text-rose-600' : 'text-[#1a1a2e]'}`} style={{ fontFamily: 'Poppins, sans-serif', letterSpacing: '-0.8px' }}>{m.tor.toFixed(2)}R</h2>
      </div>
