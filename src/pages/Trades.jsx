...
export default function Trades() {
  ...
  return (
    <div className="bg-white/70 backdrop-blur-md border-2 border-[#d0bdf4] rounded-3xl shadow-2xl overflow-hidden">
      
      {/* HEADER & FILTER */}
      <div className="p-6 bg-[#f8fafc] border-b-2 border-[#d0bdf4] flex gap-4 items-center justify-between">
        <h2 className="text-sm font-black text-[#8458B3] uppercase tracking-widest ml-2">Trade History</h2>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-center whitespace-nowrap">
          ...
          <tbody className="text-sm font-bold text-[#333] divide-y divide-[#e5eaf5]">
                ...
                <tr key={t.id} className="hover:bg-[#f8fafc] transition-colors group">
                  ...
                  <td className="py-4 px-4">
                    <div className="flex flex-col items-center">
                      <span className="text-[#333]">₹{(t.quantity * t.entry).toLocaleString()}</span>
                      <span className="text-xs text-[#a28089] mt-0.5">{t.quantity} Qty</span>
                    </div>
                  </td>
                  
                  <td className="py-4 px-4 text-[#333]">₹{t.entry}</td>
                  ...
                  <td className="py-4 px-4">
                    <div className="flex justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => setEditingTrade(t)} className="p-2 bg-white border-2 border-[#d0bdf4] rounded-lg text-[#a28089] hover:text-[#8458B3] hover:border-[#8458B3] shadow-sm transition-all" title="Edit"><Edit3 size={16} /></button>
                      {t.status === 'Open' && <button onClick={() => handleCloseTrade(t)} className="p-2 bg-white border-2 border-emerald-300 rounded-lg text-emerald-600 hover:text-emerald-700 shadow-sm transition-all" title="Close"><CheckCircle2 size={16} /></button>}
                      <button onClick={() => deleteTrade(t.id)} className="p-2 bg-white border-2 border-rose-300 rounded-lg text-rose-600 hover:text-rose-700 shadow-sm transition-all" title="Delete"><Trash2 size={16} /></button>
                    </div>
                  </td>
                </tr>
              );
