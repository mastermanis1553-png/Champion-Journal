...
export default function TradeForm() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (qty <= 0) return alert("Invalid SL!");
    await addTrade({
  ...
  return (
    <>
      ...
      <button 
        ...
        className="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white px-5 py-2.5 rounded-xl font-black text-xs uppercase tracking-widest shadow-lg shadow-blue-500/30 transition-all active:scale-95"
      >
        + Execute Trade
      </button>

      {/* MODAL WRAPPED IN isOpen (Ye hona zaroori hai) */}
      {isOpen && (
            ...
            <form onSubmit={handleSubmit} className="space-y-4 font-bold">
              ...
              <div className="bg-blue-50/50 border border-blue-100 p-4 rounded-2xl flex justify-between">
                <div className="text-center">
                  <p className="text-[9px] text-slate-400 uppercase tracking-widest">Calculated Qty</p>
